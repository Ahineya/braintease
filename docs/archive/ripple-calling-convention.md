> **Deprecated (historical).** This file is not a current specification.
> Current architecture lives in [`docs/arch/`](../arch/README.md).
> See [`docs/archive/README.md`](README.md) for why this document was archived.
...
- **PC** (1): Program counter
- **PCB** (2): Program counter bank
- **RA** (3): Return address register (used by JAL/JALR and IRQ dispatch)
- **RAB** (4): Return address bank (used by JAL/JALR and IRQ dispatch)

Hardware IRQs save the interrupted PC/PCB into RA/RAB (the instruction has not executed yet) and jump to `IRQ_VECTOR_BANK`/`IRQ_VECTOR_OFF`. Same-bank handlers may `RET`; cross-bank handlers should `JALR R0, RAB, RA`. IRQs do not nest while `IRQ_BUSY` is set. Acknowledge before returning (or keep ENABLE clear) so a still-pending IRQ cannot overwrite RA/RAB. See `rvm/mmio.md`.
